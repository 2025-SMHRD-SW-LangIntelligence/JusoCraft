  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rotatedKeys = sortedKeys
        ...
        .concat(sortedKeys.filter((key) => Number(key.slice(0, 2)) < baseHour));
          ...
          <>
            ...
            {activeFire && selectedTime && (
              <div className="absolute top-4 right-4 bg-white p-4 w-64 shadow-md rounded-lg z-[1000] text-sm">
                <h3 className="text-base font-semibold mb-1">화재 상세</h3>
                <div>{activeFire.address}</div>
                <ul>
                  <li className="flex gap-1 items-center">
                    기온 :
                    <span>
                      {activeFire.weather?.[selectedTime]?.T1H || "?"}℃
                    </span>
                  </li>
                  <li className="flex gap-1 items-center">
                    풍속 :
                    <span>
                      {activeFire.weather?.[selectedTime]?.WSD || "?"}
                      m/s
                    </span>
                  </li>
                  <li className="flex gap-1 items-center">
                    ...
                    <span>
                      {(() => {
                        const vec = parseFloat(
                          activeFire.weather?.[selectedTime]?.VEC || 0
                        );
                        const directionName = getWindDirectionText(vec);
                        const toward = (vec + 180) % 360;
                  ...
                  <li className="flex gap-1 items-center">
                    강수 :
                    <span>
                      {activeFire.weather?.[selectedTime]?.PTY === "0"
                        ? "없음"
                        : "비 또는 눈"}
                    </span>
                  </li>
                ...
                <div className="mt-2 text-xs text-gray-500">
                  기준 시각 : {selectedTime.slice(0, 2)}:{selectedTime.slice(2)}
                </div>
              </div>
            )}
            ...
            >
              ...
              {firesWithSteps.map((fire, idx) => {
                ...
                const currentStep = timeKeys.indexOf(selectedTime) + 1;
                const currentArrow = fire.arrowPointsByTime.find(
                ...
                return (
                  <React.Fragment key={idx}>
                    {/* 화살표 */}
                    {currentArrow?.arrowPoints.map((pos, i) => (
                      <Marker
                        key={`arrow-${i}`}
                        position={pos}
                    ...
                    {/* 누적 원 */}
                    {fire.stepCircles
                      .filter((s) => s.step !== 1 && s.step <= currentStep)
                      .map((s, i) => (
                        <Circle
                          key={`circle-${i}`}
                          center={s.center}
                          ...
                          pathOptions={{
                            stroke: false,
                            fillColor: s.isRain ? "#3b82f6" : "#ef4444",
                            fillOpacity: 0.8 / s.step, // step 1은 0.3, step 2는 0.15, step 3은 0.1
                          }}
                        />
                      ))}
